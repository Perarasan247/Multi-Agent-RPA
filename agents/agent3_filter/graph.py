...
from agents.agent3_filter.nodes.select_dealer import select_dealer_node
...
from agents.agent3_filter.nodes.enter_as_on_date import enter_as_on_date_node
...
def _route_after_arrow(state: GlobalState) -> str:
    """Route after opening filter panel.

    If as_on_date_only is set (e.g. Stock Valuation), set the As On Date and generate.
    If dealer is configured, go to select_dealer first.
    """
    if state.get("error"):
        return "end"
    if state.get("as_on_date_only"):
        return "as_on_date"
    if state.get("dealer"):
        return "select_dealer"
    return "continue"
...
def build_filter_graph() -> StateGraph:
    ...
    Flow:
        ...
        1b. select_dealer          — select dealer from dropdown (if configured)
    ...
    If as_on_date_only is True (e.g. Stock Valuation), steps 1b-5 are skipped
    and only the As On Date is set from FILTER_TO_DATE in .env.
    """
    graph = StateGraph(GlobalState)

    graph.add_node("click_arrow_button", click_arrow_button_node)
    graph.add_node("select_dealer", select_dealer_node)
    ...
    graph.add_node("enter_as_on_date", enter_as_on_date_node)
    graph.add_node("press_generate_report", press_generate_report_node)

    graph.set_entry_point("click_arrow_button")

    graph.add_conditional_edges(
        "click_arrow_button", _route_after_arrow,
        {
            "end": END,
            "continue": "handle_tax_checkboxes",
            "as_on_date": "enter_as_on_date",
            "select_dealer": "select_dealer",
        },
    )
    graph.add_conditional_edges(
        "select_dealer", _route,
